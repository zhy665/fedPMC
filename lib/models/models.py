...
class ProjandDeci(nn.Module):
    ...
    def copy(self):
        # 创建一个新的模型实例
        copied_model = ProjandDeci(self.fc1.in_features, self.fc1.out_features, self.fc2.out_features)
        # 将原模型的参数复制给新模型
...
class ProjandDeci_backbones(nn.Module):
    def __init__(self, in_d, out_d, num_classes):
        super(ProjandDeci_backbones, self).__init__()
        self.model1 = resnet18(pretrained=True, ds='quickdraw')
        self.model2 = resnet18(pretrained=True, ds='birds')
        self.model3 = resnet18(pretrained=True, ds='aircraft')
        # 冻结预训练模型的参数
